...
def classify_non_oil_key_issues(classification_context):
    ...
    try:
        ...
        logging.exception(
            "DeepSeek client initialization failed for non-oil key issue classification: %s",
            exc,
        )
        return with_ai_usage_meta(
            {"generated": False, "message": "AI服务初始化失败，已使用本地规则选题。", "payload": None},
            prompt_text=prompt_text,
            fallback_used=True,
        )
    if not client:
        return with_ai_usage_meta(
            {"generated": False, "message": "未配置AI服务，已使用本地规则选题。", "payload": None},
            prompt_text=prompt_text,
            fallback_used=True,
        )
    try:
        response = client.chat.completions.create(
            model=DEEPSEEK_MODEL,
            messages=[
                {"role": "system", "content": NON_OIL_KEY_ISSUE_CLASSIFICATION_SYSTEM_PROMPT},
                {"role": "user", "content": prompt},
            ],
            stream=False,
            reasoning_effort="high",
        ...
        if not isinstance(payload, dict):
            return with_ai_usage_meta(
                {"generated": False, "message": "AI重点问题结果无法识别，已使用本地规则选题。", "payload": None},
                prompt_text=prompt_text,
                completion_text=raw_content,
                ai_called=True,
                fallback_used=True,
        ...
        return with_ai_usage_meta(
            {"generated": True, "message": "AI非油重点问题分类完成。", "payload": payload},
            prompt_text=prompt_text,
            completion_text=raw_content,
            ai_called=True,
            success=True,
        ...
        status_code = get_ai_error_status_code(exc)
        logging.exception(
            "DeepSeek non-oil key issue classification failed. status=%s error=%s",
            status_code,
            exc,
        )
        return with_ai_usage_meta(
            {"generated": False, "message": "AI重点问题分类失败，已使用本地规则选题。", "payload": None},
            prompt_text=prompt_text,
            ai_called=True,
            fallback_used=True,
            status_code=status_code,
...
def generate_non_oil_report_insights(report_context):
    ...
    try:
        ...
        logging.exception("DeepSeek client initialization failed for non-oil report: %s", exc)
        return with_ai_usage_meta(
            {
                "generated": False,
                "message": "AI 服务初始化失败，已使用本地规则生成非油报告。",
                "payload": None,
            },
            prompt_text=prompt_text,
            fallback_used=True,
    ...
    if not client:
        return with_ai_usage_meta(
            {
                "generated": False,
                "message": "未配置 AI 服务，已使用本地规则生成非油报告。",
                "payload": None,
            },
            prompt_text=prompt_text,
            fallback_used=True,
    ...
    try:
        response = client.chat.completions.create(
            model=DEEPSEEK_MODEL,
            messages=[
                {"role": "system", "content": NON_OIL_REPORT_INSIGHT_SYSTEM_PROMPT},
                {"role": "user", "content": prompt},
            ],
            stream=False,
            reasoning_effort="high",
        ...
        payload = extract_json_from_ai_text(raw_content)
        if payload is None:
            logging.warning("DeepSeek returned an unusable non-oil report payload: %r", raw_content)
            return with_ai_usage_meta(
                {
                    "generated": False,
                    "message": "AI 返回内容无法识别，已使用本地规则生成非油报告。",
                    "payload": None,
                },
                prompt_text=prompt_text,
                completion_text=raw_content,
        ...
        status_code = get_ai_error_status_code(exc)
        logging.exception(
            "DeepSeek non-oil report insight generation failed. status=%s error=%s",
            status_code,
            exc,
        )
        return with_ai_usage_meta(
            {
                "generated": False,
                "message": "AI 非油报告分析失败，已使用本地规则生成报告。",
                "payload": None,
            },
            prompt_text=prompt_text,
            ai_called=True,
