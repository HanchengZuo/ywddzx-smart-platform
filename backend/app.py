...
REPORT_SNAPSHOT_TYPE_SAFETY_QUALITY = "safety_quality"
REPORT_SNAPSHOT_TYPE_FINANCE = "finance"
...
INSPECTION_REPORT_TYPE_CONFIGS = OrderedDict(
    [
        ...
        (
            ...
            {
                "key": REPORT_SNAPSHOT_TYPE_SAFETY_QUALITY,
                "name": "安全质量检查报告",
        ...
        (
            ...
            {
                "key": REPORT_SNAPSHOT_TYPE_FINANCE,
                "name": "财务检查报告",
        ...
        (
            ...
            {
                "key": REPORT_SNAPSHOT_TYPE_ON_SITE_SERVICE,
                "name": "现场服务检查报告",
        ...
        (
            ...
            {
                "key": REPORT_SNAPSHOT_TYPE_EQUIPMENT_FACILITIES,
                "name": "设备设施检查报告",
        ...
        (
            ...
            {
                "key": REPORT_SNAPSHOT_TYPE_NON_OIL,
                "name": "非油检查报告",
...
def attach_report_snapshot_meta(report, snapshot_row=None, cached=False, generated_by_name=""):
    ...
    normalized_report["title"] = str(normalized_report.get("title") or "").replace("上海销售", "")
...
def serialize_inspection_report_job(row):
    if not row:
        return None
    return {
...
def update_inspection_report_job(task_id, status, progress, stage_message, error_message=""):
    conn = None
    cur = None
    try:
        conn = get_db_connection()
        ...
        cur.execute(
            """
            UPDATE inspection_report_jobs
            SET status = %s,
                progress = %s,
                stage_message = %s,
                error_message = %s,
                started_at = CASE
                    WHEN %s IN ('running', 'completed', 'failed') THEN COALESCE(started_at, CURRENT_TIMESTAMP)
                    ELSE started_at
                END,
...
def run_inspection_report_generation_job(task_id):
    ...
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        job = get_inspection_report_job(cur, task_id)
    finally:
        close_db_resources(cur, conn)
    ...
    try:
        ...
        update_inspection_report_job(task_id, "completed", 100, "AI报告已生成")
    except Exception as exc:
        logging.exception("Inspection report background job failed. task_id=%s", task_id)
...
def start_inspection_report_generation_job(task_id):
    thread = threading.Thread(
        target=run_inspection_report_generation_job,
        args=(task_id,),
        daemon=True,
        name=f"inspection-report-{task_id[:8]}",
    )
    thread.start()
...
def queue_or_get_inspection_report_job(cur, user, report_type, report_month, force_regenerate=False):
    ...
    return None, get_inspection_report_job(cur, task_id), True


def get_authorized_inspection_report_user(cur):
    user = get_user_by_id(cur, get_authenticated_request_user_id())
    if not user:
        raise LookupError("用户不存在。")
    if not has_permission(cur, user, "view_inspection_reports"):
        raise PermissionError("当前账号无权查看AI报告。")
    return user
...
@app.route("/api/inspection-reports/status")
def get_inspection_report_status():
    report_type = str(request.args.get("report_type") or REPORT_SNAPSHOT_TYPE_QUALITY_MEASUREMENT).strip()
...
def create_inspection_report_generation_job():
    data = request.get_json(silent=True) or {}
    report_type = str(data.get("report_type") or REPORT_SNAPSHOT_TYPE_QUALITY_MEASUREMENT).strip()
    config = INSPECTION_REPORT_TYPE_CONFIGS.get(report_type)
    if not config:
        return jsonify({"success": False, "error": "报告类型不存在。"}), 400
    ...
    try:
        ...
        conn.commit()
        if created and job:
            start_inspection_report_generation_job(job.get("task_id"))
        return jsonify({
            "success": True,
            "report": report,
            "job": serialize_inspection_report_job(job),
...
@app.route("/api/inspection-reports/jobs/<task_id>")
def get_inspection_report_generation_job(task_id):
    conn = None
    cur = None
    try:
        conn = get_db_connection()
        ...
        job = get_inspection_report_job(cur, task_id)
        if not job:
            return jsonify({"success": False, "error": "报告生成任务不存在。"}), 404
        if user.get("role") != "root" and int(job.get("requested_by") or 0) != int(user.get("id") or 0):
            return jsonify({"success": False, "error": "无权查看该报告生成任务。"}), 403
        report = None
