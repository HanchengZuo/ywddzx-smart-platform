...
第十一页按随代码发布的 `standards.xlsx` 中外部规范ID关联“检查内容短语”，统计数量、占比并突出最多项；缺失映射明确标注，不编造。高频类别由AI挑选，但原因只引用同一参考表的“问题产生原因”原文。高频面板只读展示完整关联证据；特性与严重问题面板支持按站点、描述、ID筛选并人工改选。特性问题排除已选高频类别。选择全局共享、记录最后保存人，保存不调用AI，重新生成才应用。
