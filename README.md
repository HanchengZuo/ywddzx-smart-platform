...
flowchart TD
    ...
    RECTIFY -->|指定五张表，可填写理由申诉| APPEAL[申诉中：暂停整改，进入申诉空间]
...
- 两级审核均需原因。任何一级拒绝后回到待整改，可重新申诉；质安部通过后标记已销毁，同时退出审核通过的数据统计口径，不物理删除。已结束申诉不在待处理列表，保留只读历史查询。
