...
flowchart TD
    ...
    STATION_RESULT -->|站经无法整改| RNOTE[整改说明，无需照片]
    RPHOTO --> REVIEW[进入督导组待复核]
    RNOTE --> REVIEW

    REVIEW --> REVIEW_RESULT{督导组复核结论}
    REVIEW_RESULT -->|已整改| CLOSED[已闭环，必传复核照片]
    REVIEW_RESULT -->|站经无法整改| UNABLE[站经无法整改，流程结束]
    REVIEW_RESULT -->|整改不通过| RETURN[不要复核照片，复核说明选填]
    RETURN --> RECTIFY
```
...
2. 自动审核可使用“外部规范 ID 全匹配”或“问题描述关键词”作为触发条件，规则可配置通过、否决和优先级。
...
| 站点整改 | 站经无法整改 | 必填 | 无需照片 | 待复核 |
| 督导复核 | 已整改 | 选填 | 必传复核照片 | 已闭环 |
| 督导复核 | 站经无法整改 | 选填 | 无需照片 | 站经无法整改 |
| 督导复核 | 整改不通过 | 选填，建议说明退回原因 | 无需照片 | 退回待整改 |

整改复核记录遵循以下原则：

1. 每次“提交整改”和“提交复核”都写入独立历史记录，保留轮次、操作人、时间、前后状态、结果、说明和照片。
2. “整改不通过”会将问题退回对应站点账号，并恢复待整改提醒数量。
3. 站点再次提交时，当前复核字段会更新，但以往轮次不会被覆盖。
4. “我的待整改 / 待复核”和“巡检问题列表”均可查看完整流转时间线。
...
| 巡检问题列表 | 按数据范围查询问题，审核、编辑、删除、更换检查人、设置优秀问题、查看流转记录和后台导出 |
| 巡检记录 | 按站点与检查表查看检查结果、参与人进度、问题审核进度、站经理签名和整改进度 |
| 我的待整改 / 待复核 | 站点账号处理本站待整改问题；督导账号处理待复核问题 |
...
| 白名单自动审核管理 | 按外部规范 ID 或问题描述关键词设置自动通过 / 否决规则，并提供回溯导出 |
...
2. 巡检问题保留问题照片、整改照片、复核照片以及对应的提交时间。
3. 整改和复核采用追加历史记录，不会因再次退回或提交而丢失上一轮信息。
