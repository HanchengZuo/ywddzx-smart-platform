...
仅适用计量稽查检查表（视频/现场）、环境无异味管理检查表（现场）、质量安全环保检查表（视频/现场）。root 通过“管理系统 → 质安流程时限管理”（`/management/quality-deadlines`）调整三个环节的天数（1至365的整数），并查询按巡检记录ID、问题ID、事件类型筛选的追溯日志；普通角色不能访问管理接口，身份以服务端当前登录用户为准。
...
- 期限采用连续24小时计天，按北京时间展示。各阶段保存规则快照，root之后的修改只对新进入环节的任务生效，不能缩短或延长已有任务的期限；初始存量待办从迁移启用时刻获得完整期限，不因旧日期立即超时。
...
- 部署必须先执行 Alembic 至 `20260906_002`。Docker入口先迁移和验证结构，再用 Gunicorn `post_worker_init` 启动后台扫描（约每30秒，分批处理）；无需访问页面。多worker/多实例通过PostgreSQL事务锁防重，失败整批回滚并重试，管理页显示最近扫描成功/失败时间。大量存量任务首次初始化可能需要数轮扫描。
